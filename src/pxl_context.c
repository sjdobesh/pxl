...
/**
 * initialize default window and eenderer settings.
 *
...
int PXL_CreateSDLResources(PXL_Context *context) {
  ...
  if (PXL_CreateWindowRenderer(context))
    return 1;

  if (PXL_CreateFontEngine(context))
    return 1;
...
int PXL_CreateScreen(PXL_Context *context) {
  ...
  if (PXL_CreateTextureFromSurface(context->renderer, context->screen_surface,
...
int PXL_CreateMenu(PXL_Context *context) {
  ...
  if (PXL_CreateTextureFromSurface(context->renderer, context->text_surface,
                                   &(context->text_texture)))
    return 1;

  if (PXL_CreateMenuText(context))
    return 1;
...
int PXL_Init(PXL_Context *context) {
  ...
  if (PXL_CreateContextResources(context))
    return 1;
...
int PXL_Update(PXL_Context *context) {
  ...
  /* &=~ and not this, set everything else as it was except this bit */

  if (context->update & SCREEN_UPDATE) {
    if (PXL_UpdateScreenTexture(context))
      return 1;
    context->update &= ~SCREEN_UPDATE;
    context->render = 1;
  }
  ...
  if (context->update & MENU_UPDATE) {
    if (PXL_UpdateMenuTexture(context))
      return 1;
    context->update &= ~MENU_UPDATE;
    context->render = 1;
  }
...
int PXL_HandleEvents(PXL_Context *context, SDL_Event *event) {
  ...
  if (context->menu_selection < MENU_LENGTH - 1 &&
      context->kbstate[SDL_SCANCODE_L]) {
    context->menu_selection++;
  }
  return SDL_APP_CONTINUE;
}
...
int PXL_Render(PXL_Context *context) {
  ...
  if (!SDL_RenderClear(context->renderer))
    return 1;
  if (PXL_RenderScreen(context) || PXL_RenderText(context))
    return 1;
